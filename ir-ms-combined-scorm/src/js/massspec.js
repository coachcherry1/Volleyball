 ...
 *
 * Copied from the mass spectrometry package. Two changes here: LABEL_ATOM
 * knows an explicit aldehyde 'H' and a 'NO₂' group, which the IR bank's
 * structures use, and neutralOf() spells three more losses the conventional
 * way (HCN, •COOH, •CO₂CH₃).
 *
...
var MS = (function () {
  ...
  /* Two labels arrived with the IR bank. 'H' is an aldehyde hydrogen drawn
     explicitly, the way the IR package draws it — without it propanal and
     acetone look alike on a candidate card. 'NO₂' is a whole group drawn as
     text; `extra` carries the atoms the label stands for beyond its first. */
  var LABEL_ATOM = {
  ...
  function atomsOf(structure) {
    ...
    for (var v = 0; v < n; v++) {
      if (labels[v]) {
        ...
        atoms.push(L ? { el: L.el, h: L.h, extra: L.extra || null } : null);
      } else {
  ...
  function countsOf(atoms, members) {
    ...
    members.forEach(function (v) {
      ...
      if (a.extra) for (var x in a.extra) c[x] = (c[x] || 0) + a.extra[x];
    });
    return c;
  }
