/* fragments.js — the mass spectrum label vocabulary.
 *
 * Copied from the mass spectrometry package, with three kinds of change:
 *
 *   - the helper functions carry an `ms` prefix (msKey, msLabel, msSub,
 *     msHint, MS_UNIVERSE), because groups.js defines its own keyLabel and
 *     UNIVERSE for the IR spectrum and both now load into one page;
 *   - five losses the new compounds need: −27 HCN, −28 CO or C₂H₄, −46 NO₂,
 *     −59 CO₂CH₃, and •COOH as the second reading of −45;
 *   - five fragments that go with them.
 *
 * Every label a student drags works in the LOSS view — "that peak is M minus
 * 43" — because that is how the chemistry is taught: fragments lost from the
 * molecular ion. The FRAGMENT view ("that 43 is CH3CO+") is recognition
 * material only: it shows up in the reference table and in feedback, but no
 * student is asked to produce one.
 *
 * The loss is arithmetic: M - m/z. The ion is chemistry. CH3CO+ sits at m/z
 * 43 in BOTH acetone and 2-butanone, but it is a loss of 15 from one and a
 * loss of 29 from the other. So a loss key is COMPUTED per molecule (see
 * lossKey below) rather than stored on the ion.
 *
 * MECHANISMS. Four, and no more — the same four the mass spec package
 * teaches. Real peaks from anything else (McLafferty, ring contractions, the
 * NO loss from a nitro group) are drawn, because deleting a real peak would
 * be a lie, but they are never scored and each explains itself when clicked.
 *
 *   alpha       a bond next to a heteroatom breaks, and the lone pair left
 *               behind stabilises the cation by resonance
 ...
 *   sigma       a plain break with nothing special stabilising it
 */
...
var LOSSES = {
  ...
  17: { neutral: '•OH',     hint: 'Loss of a hydroxyl radical. In a carboxylic acid it is a big peak — the acid drops its •OH and keeps the C=O. In an alcohol it is usually much weaker than the −18 next to it.' },
  ...
  27: { neutral: 'HCN',     hint: 'Loss of hydrogen cyanide, a whole small molecule. A nitrogen on or in a benzene ring — a nitrile or an aromatic amine — throws off HCN. The odd molecular ion said nitrogen; this says where it was.' },
  28: { neutral: 'CO', also: 'C₂H₄',
        hint: 'Loss of 28 is AMBIGUOUS: carbon monoxide, or a molecule of ethene. A ring that cannot lose a radical without breaking two bonds sheds a whole small molecule instead — CO from a phenol, C₂H₄ from a cycloalkane. The IR decides it: is there an O–H or a C=O?' },
  ...
  45: { neutral: '•OC₂H₅', also: '•COOH',
        hint: 'Loss of 45 is AMBIGUOUS: an ethoxy radical from an ethyl ester, or the whole •COOH group from a carboxylic acid. The IR settles it in one look — an acid has the enormous O–H envelope, an ester has none.' },
  46: { neutral: '•NO₂',    hint: 'Loss of a nitro group. The molecular ion was odd, so there is a nitrogen; losing 46 says it came as NO₂. The IR agrees: a very strong band near 1520.' },
  ...
  59: { neutral: '•CO₂CH₃', hint: 'Loss of the whole methyl ester group, •COOCH₃. It leaves the bare ring behind when the ester sits on one — so a −59 next to a −31 from the same molecular ion is a methyl ester.' },
...
var IONS = {
  ...
  cooh: {
    f: 'CHO2', mz: 45, cls: 'resonance', mech: 'alpha', label: 'HO–C≡O⁺', name: 'keeps the C=O and the OH', twin: 'ch3choh',
    hint: 'ALPHA CLEAVAGE at an acid: whatever sits next to the C=O leaves, and the COOH keeps the charge. m/z 45 with a strong M−17 beside it points to a carboxylic acid. Careful — an oxygen fragment from an alcohol or ether weighs 45 too.'
  },
  ...
  conh2: {
    f: 'CH2NO', mz: 44, cls: 'resonance', mech: 'alpha', label: 'H₂N–C≡O⁺', name: 'keeps the C=O and the nitrogen',
    hint: 'ALPHA CLEAVAGE at a primary amide: the group on the other side of the C=O leaves, and both the C=O and the nitrogen help hold the charge. m/z 44 as the base peak is the sign of a –CONH₂.'
  },
  ch3nhco: {
    f: 'C2H4NO', mz: 58, cls: 'resonance', mech: 'alpha', label: 'CH₃NH–C≡O⁺', name: 'keeps the C=O and the nitrogen',
    hint: 'ALPHA CLEAVAGE at an N-methyl amide: the methyl on the carbonyl side leaves, and the C=O and the nitrogen share the charge.'
  },
  ...
  c6h4: {
    f: 'C6H4', mz: 76, cls: 'radical', mech: 'sigma', label: 'C₆H₄⁺·', name: 'the ring, after HCN leaves',
    hint: 'What is left when a benzene ring carrying a nitrogen throws off HCN, 27. A whole neutral molecule left, so what remains is still a radical cation.'
  },
  c4h8: {
    f: 'C4H8', mz: 56, cls: 'radical', mech: 'sigma', label: 'C₄H₈⁺·', name: 'the ring, after ethene leaves',
    hint: 'A cycloalkane ring opens and throws off a whole molecule of ethene, C₂H₄. A ring has to break two bonds to lose anything, so it sheds a small molecule rather than a radical.'
  },
...
/* Every mass spectrum label is a LOSS, which only exists relative to the
   molecular ion, so it has to be worked out per molecule rather than stored.
   The landmarks answer to themselves. */
function msKey(peak, molecularMass) {
...
/* Everything a tile may show. Only the loss vocabulary is draggable: the
   ...
   reference table and in the feedback — but a student is never asked to
   produce one. */
var MS_UNIVERSE = ['mplus', 'miso1', 'miso2'].concat(
